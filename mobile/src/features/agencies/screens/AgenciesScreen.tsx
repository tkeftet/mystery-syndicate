...
export function AgenciesScreen() {
  ...
  function manageMember(m: any) {
    ...
    if (RANK[myRole] >= RANK.coleader) {
      if (m.role === "member")
        buttons.push({ label: "Promote to Officer", variant: "primary", onPress: () => { setPopup(null); run(() => setMemberRoleApi(m.userId, "officer")); } });
      if (m.role === "officer")
        buttons.push({ label: "Demote to Member", variant: "secondary", onPress: () => { setPopup(null); run(() => setMemberRoleApi(m.userId, "member")); } });
    }
    if (myRole === "leader")
      buttons.push({ label: "Transfer Leadership", variant: "secondary", onPress: () => { setPopup(null); run(() => transferLeadershipApi(m.userId)); } });
    buttons.push({ label: "Kick", variant: "danger", onPress: () => { setPopup(null); run(() => kickMemberApi(m.userId)); } });
    buttons.push({ label: "Cancel", variant: "secondary", onPress: () => setPopup(null) });
    setPopup({ title: m.username, message: ROLE_LABEL[m.role], buttons });
  }
  ...
  return (
    <View style={[styles.safeTop, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.kicker}>// AGENCIES</Text>
          <Text style={styles.headerTitle}>Detective Agencies</Text>
        </View>
        <TouchableOpacity
        ...
        {inAgency ? (
          <>
            ...
            <LinearGradient colors={gradients.seal} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[styles.banner, shadows.glow]}>
              ...
              <Text style={styles.bannerDesc} numberOfLines={2}>{mine.agency.description || "No description."}</Text>
              <View style={styles.bannerStats}>
                <View style={styles.bStat}><Text style={styles.bVal}>Lv {mine.agency.level}</Text><Text style={styles.bLbl}>LEVEL</Text></View>
                <View style={styles.bStat}><Text style={styles.bVal}>{mine.agency.weeklyPoints}</Text><Text style={styles.bLbl}>WEEKLY</Text></View>
                <View style={styles.bStat}><Text style={styles.bVal}>{mine.agency.memberCount}/{mine.agency.memberCap}</Text><Text style={styles.bLbl}>MEMBERS</Text></View>
              </View>
                ...
                {requests.map((r: any) => (
                  <View key={r.userId} style={styles.row}>
                    ...
                    <TouchableOpacity style={[styles.pill, styles.pillPrimary]} onPress={() => run(() => approveRequestApi(r.userId))}>
                      <Text style={styles.pillPrimaryText}>Approve</Text>
                    </TouchableOpacity>
            ...
            {/* Members */}
            <Text style={styles.sectionLabel}>MEMBERS ({mine.members.length})</Text>
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.rowSub}>
                    {ROLE_LABEL[m.role]} · {m.contributionTotal} pts
                  </Text>
                </View>
              </TouchableOpacity>
            ))}
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                setPopup({
                  title: myRole === "leader" ? "Leave / Delete agency" : "Leave agency?",
                  message:
                    myRole === "leader"
                      ? "As leader you must transfer leadership first, or delete the agency entirely."
                      : "You'll lose your role and contribution standing.",
                  buttons:
                    myRole === "leader"
                      ? [
                          { label: "Delete Agency", variant: "danger", onPress: () => { setPopup(null); run(() => deleteAgencyApi()); } },
                          { label: "Cancel", variant: "secondary", onPress: () => setPopup(null) },
                        ]
                      : [
                          { label: "Leave", variant: "danger", onPress: () => { setPopup(null); run(() => leaveAgencyApi()); } },
                          { label: "Cancel", variant: "secondary", onPress: () => setPopup(null) },
                        ],
                })
              }
            >
              <Text style={styles.dangerText}>
                {myRole === "leader" ? "Leave / Delete Agency" : "Leave Agency"}
              </Text>
            </TouchableOpacity>
          </>
        ) : (
          <>
            ...
            >
              ...
              <Text style={styles.createText}>Create an Agency</Text>
            </TouchableOpacity>
              ...
              list.results.map((a: any) => (
                <View key={a.id} style={styles.row}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.rowSub}>
                      Lv {a.level} · {a.memberCount}/{a.memberCap} · {a.weeklyPoints} wk
                    </Text>
                  </View>
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      run(async () => {
                        ...
                        if (res?.status === "requested")
                          setPopup({ title: "Request sent", message: "An officer will review your request." });
                      })
                    }
                  >
                    <Text style={styles.pillPrimaryText}>
                      {a.privacy === "request" ? "Request" : "Join"}
                    </Text>
                  </TouchableOpacity>
                </View>
              ))
            ) : (
              <Text style={styles.empty}>No agencies found. Create the first one!</Text>
            )}
          </>
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  onlineDot: { position: "absolute", right: -1, bottom: -1, width: 10, height: 10, borderRadius: 5, backgroundColor: colors.green, borderWidth: 2, borderColor: colors.bg.secondary },
