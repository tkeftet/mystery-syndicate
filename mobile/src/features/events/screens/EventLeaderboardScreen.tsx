...
import { useEventLeaderboard, useMyParticipation } from "../events.hooks";
...
export function EventLeaderboardScreen({ route }: { route: Rt }) {
  ...
  const { data: board, isLoading } = useEventLeaderboard(eventId);
  const { data: mine } = useMyParticipation(eventId);

  const myUserId = mine?.participation?.userId;
